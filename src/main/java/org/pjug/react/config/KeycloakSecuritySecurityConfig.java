...
public class KeycloakSecuritySecurityConfig {
    ...
    @Bean
    public SecurityFilterChain keycloakSecurityFilterChain(final HttpSecurity http) throws
            Exception {
        return http.cors(withDefaults())
                .csrf(csrf -> csrf.disable())
                .headers(headers -> headers.frameOptions(frameOptions -> frameOptions.sameOrigin()))
                .authorizeHttpRequests(authorize -> authorize
                    .requestMatchers(EndpointRequest.toAnyEndpoint()).hasAuthority(UserRoles.ROLE_ADMIN)
