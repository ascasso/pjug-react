...
export default function GroupMeetingAdd() {
  const { t } = useTranslation();
  useDocumentTitle(t('groupMeeting.add.headline'));
  ...
  const createGroupMeeting = async (data: GroupMeetingDTO) => {
    window.scrollTo(0, 0);
    try {
      await axios.post('/api/groupMeetings', data);
      navigate('/groupMeetings', {
            state: {
              msgSuccess: t('groupMeeting.create.success')
            }
          });
    } catch (error: any) {
  ...
  return (<>
    <div className="d-flex flex-wrap mb-4">
      <h1 className="flex-grow-1">{t('groupMeeting.add.headline')}</h1>
      <div>
        <Link to="/groupMeetings" className="btn btn-secondary">{t('groupMeeting.add.back')}</Link>
      </div>
    </div>
    <form onSubmit={useFormResult.handleSubmit(createGroupMeeting)} noValidate>
      ...
      <input type="submit" value={t('groupMeeting.add.headline')} className="btn btn-primary mt-4" />
    </form>
  </>);
}
